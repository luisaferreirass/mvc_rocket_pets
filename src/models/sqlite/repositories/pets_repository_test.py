...
class MockConnection: # Essa classe não possui erro
...
class MockConnectionNoResult: # Essa classe possui erro
    def __init__(self) -> None:
        self.session = UnifiedAlchemyMagicMock()
        self.session.query.side_effect = self.__raise_no_result_found
        # É o side effect que lança para gente a função do nosso erro
        # que vai ser executado assim que a gente executar o query

    def __raise_no_result_found(self, *args, **kwargs): 
        # Todas as propriedades enviadas para la para cima que forem recebidas aqui
...
def test_delete_pets_error():
    ...
    with pytest.raises(Exception): # Estamos conferindo mesmo se foi lançado o erro
        repo.delete_pets("petName")
